...
function Product({products}:productProps) {
    ...
    const [filterItems,setFilterItems] = useState<VariantsWithProduct[]>([])
    useEffect(()=>{
    if(tagParam){
      const itemsFilter = products.filter((p)=>p.variantTags[0].tag===tagParam)
  ...
  return (
    ...
    {
      filterItems.length>0 ? filterItems.map((p)=>(
